...
local EatSelect = class(  "EatSelect" )

EAT_MAX_STACK = 3   -- 最大组数
EAT_MAX_CARDS = 3   -- 单组牌数

-- 构造
function EatSelect:ctor( node, callback )
...
-- 显示吃牌面板
function EatSelect:show( stacks, defIndex )

    -- 堆数组不能超过3组
    if not stacks or #stacks > EAT_MAX_STACK then
        g_methods:error( "吃牌提示显示失败,无效的提示信息!" )
        return
    end    
    
    -- 默认聚焦
    if defIndex < 1 or defIndex > #stacks then
        g_methods:error( "吃牌提示显示失败,默认聚焦无效!" )
        return
    end
    ...
    for i = 1, EAT_MAX_STACK do
        ...
        if stackIdx > 0 and stackIdx <= #stacks then 
            ...
            if #cardlst ~= EAT_MAX_CARDS then
                g_methods:error( "吃牌提示显示失败,第%d组提示张数错误:%d!", i, #cardlst )
                return
            end
...
-- 改变聚焦
function EatSelect:changeFocus( idx )
...
-- 聚焦检测
function EatSelect:freshFocus( loca )
...
-- 鼠标按下
function EatSelect:onMouseTouchDown( location, args )
...
-- 鼠标弹起
function EatSelect:onMouseTouchUp( location, args )
...
-- 鼠标滚动
function EatSelect:onMouseTouchMove( location, args )
